...
func Init_route_to_message(router *iris.Application, instance_manager *proxy.Service_instance_manager, endpoint_manager *proxy.Service_endpoint_manager) {
	an_proxy := Proxy_to_keystone{
		service_name:     api.MS_http_server_message,
		transport_type:   "http",
		router:           router,
		instance_manager: instance_manager,
		endpoint_manager: endpoint_manager}

	instance_manager.Register_instance(api.MS_http_server_message)

	_ = an_proxy.Make_route("POST", api.EDP_http_message__room__check_room)
}

func (p *Proxy_to_message) Make_route(method, path string) error {
